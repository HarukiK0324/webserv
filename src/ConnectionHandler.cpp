...
ConnectionHandler ::ConnectionHandler(int fd, ServerLoop &loop, std::map< std::string, ServerConfig > &conf, const std::string &client_ip) : FdHandler(fd, POLLIN, loop),
...
void ConnectionHandler ::CheckTimeout(const time_t &_time)
{
	...
	{
		std::cerr << "ConnectionHandler: Connection timed out." << std::endl;
		std::cout << "Time out connection fd = " << _fd << std::endl;
		SetState(TIMEOUT);
		SetEventPollout();
	}
}
...
void ConnectionHandler ::ReadEvent(Event ev)
{
	...
	while (1)
	{
		...
		_httpProtocol._inbuf.resize(BUFSIZ);
		ssize_t n = recv(_fd, _httpProtocol._inbuf.data(), BUFSIZ, 0);
...
void ConnectionHandler ::WriteEvent(Event ev)
{
	...
	switch (_state)
	{
		...
		case TIMEOUT:
		{
			std::cerr << "ConnectionHandler: Handling timeout event." << std::endl;
...
// void ConnectionHandler ::SetLastTime()
// {
// 	_lastActive = (std::time(NULL));
// }