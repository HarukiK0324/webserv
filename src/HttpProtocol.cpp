	...
	switch (getState())
	{
		case HttpProtocol::PARSING:
		{
			...
			else if (_httpParser.getState() == DONE)
			{
				...
				bool session_valid = is_existing
					&& _loop.getSessions(session_id).getExpiresAt() >= std::time(NULL);
				if (!session_valid)
				{
					if (is_existing)
					{
		...
		case HttpProtocol::METHOD_PROCESSING:
		{
			...
			if (IsCgi(_httpRequest, lconfig))  //CGI
			{
				...
				_cgiHandler->MakeEnvp(_httpRequest, _client_ip, to_string(sconf.port));
				SetState(WAIT_CGI);
			}
			else  //Static Method
			{
