#include "../include/HeaderFieldReader.hpp"
#include <iostream>
...
{
    size_t pos;
    while((pos = raw_request.find(CRLF)) != raw_request.find(std::string(CRLF) + CRLF) && pos != std::string::npos)
    {
    ...
    if(raw_request.find(std::string(CRLF) + CRLF) == std::string::npos)
        return false;
    raw_request.erase(0, raw_request.find(std::string(CRLF) + CRLF) + 4);
